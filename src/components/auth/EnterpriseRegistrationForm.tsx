...
const EnterpriseRegistrationForm = ({ 
  email, 
  submissions, 
  pattern, 
  onSuccess 
}: EnterpriseRegistrationFormProps) => {
    ...
    try {
      ...
      if (result.success) {
        toast({
          ...
          description: `Welcome! Your ${submissions.length} business unit analyses have been linked.`,
        });
    ...
    <div className="space-y-6">
      ...
      <Card className="border-revenue-warning/20 bg-revenue-warning/5">
        <CardContent className="p-4">
          <div className="flex items-center gap-3 mb-3">
            <Building className="h-5 w-5 text-revenue-warning" />
            <h3 className="font-semibold text-revenue-warning">Enterprise Multi-Division Detected</h3>
          </div>
          <p className="text-sm text-muted-foreground mb-3">
            You've analyzed {submissions.length} different business units or divisions. 
            Let's set up your enterprise account for comprehensive revenue management.
          </p>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <div className="text-center">
              <div className="text-lg font-bold text-revenue-warning">{submissions.length}</div>
              <div className="text-xs text-muted-foreground">Business Units</div>
            </div>
            <div className="text-center">
              <div className="text-lg font-bold text-revenue-warning">{pattern?.unique_industries || 0}</div>
              <div className="text-xs text-muted-foreground">Industries</div>
            </div>
            <div className="text-center">
              <div className="text-lg font-bold text-revenue-warning">{formatCurrency(totalPortfolioValue)}</div>
              <div className="text-xs text-muted-foreground">Combined ARR</div>
            </div>
          </div>
        </CardContent>
      </Card>
      ...
      <Card className="border-border/50 shadow-lg">
        <CardHeader>
          ...
          <CardDescription>
            Managing multiple business units? Get enterprise-level revenue insights
          </CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="space-y-4">
              <h4 className="font-medium">Corporate Information</h4>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                  <Label htmlFor="actualRole">Your Corporate Role</Label>
                  <Select value={formData.actualRole} onValueChange={(value) => setFormData({...formData, actualRole: value})}>
                    ...
                    <SelectContent>
                      ...
                      <SelectItem value="vp_revenue_ops">VP Revenue Operations</SelectItem>
                      <SelectItem value="vp_finance">VP Finance</SelectItem>
                      <SelectItem value="coo">Chief Operating Officer</SelectItem>
                      <SelectItem value="division_manager">Division Manager</SelectItem>
                      <SelectItem value="director_ops">Director of Operations</SelectItem>
            ...
            <Button
              type="submit"
              className="w-full bg-gradient-to-r from-revenue-warning to-revenue-primary"
              disabled={loading}
            >
      ...
      <Card className="border-border/50">
        <CardHeader>
          <CardTitle className="text-lg">Your Business Units</CardTitle>
          <CardDescription>Divisions and units you've analyzed will be linked to your account</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="space-y-3">
                ...
                <div key={index} className="flex items-center justify-between p-3 bg-muted/30 rounded-lg">
                  ...
                  <div className="text-right text-sm">
                    ...
                    {submission?.total_leak && (
                      <div className="text-revenue-warning text-xs">
            ...
            {uniqueCompanies.length > 5 && (
              <div className="text-center text-sm text-muted-foreground">
                +{uniqueCompanies.length - 5} more business units
              </div>
            )}
          </div>
        </CardContent>
